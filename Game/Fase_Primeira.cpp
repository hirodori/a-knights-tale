...
Fase_Primeira::Fase_Primeira(const int i, Cavaleiro* pJ1, Escudeiro* pJ2, G_Vetor2f tamanhoBack) :
	Fase(i, pJ1, pJ2, tamanhoBack), checkpoint(0.f, 0.f)
{
	body.setTexture("Assets//Fase01.png");
...
void Fase_Primeira::inicializarMapa()
{
	...
	for (int i = 0; i < 23; i++) { //altura
		for (int j = 0; j < 30; j++) { //largura
			...
			if (faseMapa[i][j] == ' ') {
				...
				checkpoint.criar(425.f + 50.f * j, 425.f + 50.f * i);
			}
		}
...
void Fase_Primeira::executar()
{
	...
	if (pJogador1 != NULL) {
		if((pJogador1->getCorpo()->getPosition().getX() >= checkpoint.getX()) && (pJogador1->getCorpo()->getPosition().getY() >= checkpoint.getY())){
			this->setFaseConcluida(true);
		}
	}
	if (pJogador2 != NULL) {
		if ((pJogador2->getCorpo()->getPosition().getX() >= checkpoint.getX() - 10.f) && (pJogador2->getCorpo()->getPosition().getY() >= checkpoint.getY() - 10.f)) {
			this->setFaseConcluida(true);
		}
	}
